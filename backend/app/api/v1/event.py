...
from app.services.event import find_event, add_player as event_service_add_player
...
async def add_player(event_id: uuid.UUID, player: PlayerIdModel, session: AsyncSessionDep):
    event_obj = await find_event(event_id, session)
    if not event_obj:
        raise HTTPException(
    ...
    updated_event = await event_service_add_player(player_obj.id, event_obj.id, session)
